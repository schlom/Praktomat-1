...
class MainNeedHelper(models.Model):
    """ Abstract class supporting infrastructure on django admin web interface for checking if a main method or function is necessary """

    class Meta(Checker.Meta):
        abstract = True

    # elements for using via django admin web interface
    ...
    def main_required(self):
        """ Returns true, if Trainer has chosen, that a submission have to provide a main. """
...
class LibraryHelper(models.Model):
    """ Abstract class supporting infrastructure on django admin web interface for handling libraries via compiler or linker """
...
class IncludeHelper(models.Model):
    """ Abstract class supporting infrastructure on django admin web interface for handling additional search-paths while interacting with compiler or linker """
...
class CompilerOrLinker(Checker, IncludeHelper):
    ...
    # do not override in subclass (dont hack name mangling)
    __output_flags  = ""
    __flags        = ""
...
#            raise TypeError
    ...
    def environment(self):
        """ Environment to be set on invocation of Compiler. """
        return self._env
...
class Compiler(CompilerOrLinker):
    ...
    _compiler    = "compiler_configurable"        # command to invoke the compiler in the shell
