...
class DiffChecker(Checker):
    ...
    def clean(self):
        ...
        if (not self.shell_script or not self.input_file or not self.output_file): raise ValidationError(
            _("Robert says: DiffChecker have to have an Shell script, an Inputfile and an Outputfile"))

    def title(self):
        """ Returns the title for this checker category. """
        return "Ausgaben mit 'diff' prüfen."

    @staticmethod
    def description():
        """ Returns a description for this Checker. """
        return "Diese Prüfung wird bestanden, wenn erwartete und tatsächliche Ausgabe übereinstimmen."
